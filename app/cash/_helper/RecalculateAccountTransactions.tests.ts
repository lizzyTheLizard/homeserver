...
describe('loadJournal', () => {
  ...
  test('Multiple Transactions', async ({ task }) => {
    ...
    const beforePeriodTransaction = { id: randomUUID(), project_id: project.id, credit_account_id: account1.id, debit_account_id: account2.id, amount: 200.00, date: new Date('2023-04-30'), description: 'Before period transaction' }
    const beforeDateTransaction = { id: randomUUID(), project_id: project.id, credit_account_id: account1.id, debit_account_id: account2.id, amount: 300.00, date: new Date('2023-05-01'), description: 'Before date transaction' }
    ...
    await transactional(async tx => recalculateTransactions(tx, task.id, project.id, transaction1.date, [account1.id]))
    ...
    expect(result[0]).toEqual(expect.objectContaining({
      ...
      amount: 0,
      total_balance: -transaction1.amount - transaction2.amount,
    ...
    expect(result[2]).toEqual(expect.objectContaining({
      ...
      total_balance: -transaction1.amount,
