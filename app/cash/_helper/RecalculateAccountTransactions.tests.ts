...
describe('loadJournal', () => {
  test('No Transactions', async ({ task }) => {
    ...
    await transactional(async tx => recalculateTransactions(tx, task.id, project.id, new Date('2023-05-01'), [account1.id]))

    const result = await nontransactional(c => findAllAccountTransactionsInPeriod(c, task.id, project.id, account1.id, { year: 2023, month: 5, openEnded: true }))
  ...
  test('Single Closing', async ({ task }) => {
    ...
    const account1 = { id: randomUUID(), project_id: project.id, name: 'Cash', type: 'Asset', archived: false } as AccountInput
    const account2 = { id: randomUUID(), project_id: project.id, name: 'Revenue', type: 'Income', archived: false } as AccountInput
    const closing = { id: randomUUID(), project_id: project.id, date: new Date('2023-01-31'), capital_account_id: account1.id, profit_account_id: account2.id, profit: 200.75 }
    await transactional(async (tx) => {
      await createOrModifyProject(tx, project)
      await createOrModifyAccount(tx, task.id, account1)
      await createOrModifyAccount(tx, task.id, account2)
    ...
    const result = await nontransactional(c => findAllAccountTransactionsInPeriod(c, task.id, project.id, account1.id, { year: 2023, month: 1, openEnded: true }))
    expect(result).toEqual([expect.objectContaining({
      ...
      description: 'Closing on ' + closing.date.toISOString().split('T')[0],
    })])
  })
  ...
  test('Multiple Transactions', async ({ task }) => {
    ...
    const closing1 = { id: randomUUID(), project_id: project.id, date: new Date('2023-01-31'), capital_account_id: account1.id, profit_account_id: account2.id, profit: 200 }
    const closing2 = { id: randomUUID(), project_id: project.id, date: new Date('2023-05-31'), capital_account_id: account1.id, profit_account_id: account2.id, profit: -100 }
    const transaction1 = { id: randomUUID(), project_id: project.id, credit_account_id: account1.id, debit_account_id: account2.id, amount: 100.50, date: new Date('2023-05-15'), description: 'Test transaction' }
    const transaction2 = { id: randomUUID(), project_id: project.id, credit_account_id: account1.id, debit_account_id: account2.id, amount: 50.25, date: new Date('2023-05-20'), description: 'Second transaction' }
    const otherAccountTransaction = { id: randomUUID(), project_id: project.id, credit_account_id: account2.id, debit_account_id: account3.id, amount: 75.00, date: new Date('2023-05-18'), description: 'Other account transaction' }
    const beforePeriodTransaction = { id: randomUUID(), project_id: project.id, credit_account_id: account1.id, debit_account_id: account2.id, amount: 200.00, date: new Date('2022-04-30'), description: 'Before period transaction' }
    const beforeDateTransaction = { id: randomUUID(), project_id: project.id, credit_account_id: account1.id, debit_account_id: account2.id, amount: 300.00, date: new Date('2023-01-01'), description: 'Before date transaction' }
    const doubleAccountTransaction = { id: randomUUID(), project_id: project.id, credit_account_id: account1.id, debit_account_id: account1.id, amount: 25.00, date: new Date('2023-05-25'), description: 'Double account transaction' }
    await transactional(async (tx) => {
      await createOrModifyProject(tx, project)
      await createOrModifyAccount(tx, task.id, account1)
      await createOrModifyAccount(tx, task.id, account2)
  ...
  test('Credit Account', async ({ task }) => {
    ...
    const account1 = { id: randomUUID(), project_id: project.id, name: 'A1', type: 'Asset', archived: false } as AccountInput
    const account2 = { id: randomUUID(), project_id: project.id, name: 'A2', type: 'Income', archived: false } as AccountInput
    const transaction = { id: randomUUID(), project_id: project.id, credit_account_id: account1.id, debit_account_id: account2.id, amount: 100.50, date: new Date('2023-05-15'), description: 'Test transaction' }
    await transactional(async (tx) => {
      await createOrModifyProject(tx, project)
      await createOrModifyAccount(tx, task.id, account1)
      await createOrModifyAccount(tx, task.id, account2)
  ...
  test('Existing Account Transaction', async ({ task }) => {
    ...
    const account2 = { id: randomUUID(), project_id: project.id, name: 'Revenue', type: 'Income', archived: false } as AccountInput
    const transaction = { id: randomUUID(), project_id: project.id, credit_account_id: account2.id, debit_account_id: account1.id, amount: 100.50, date: new Date('2023-05-15'), description: 'Test transaction' }
    const accountTransaction = { id: randomUUID(), account_id: account1.id, date: new Date('2023-05-13'), ordering: 13, other_account_id: account2.id, amount: 500.00, total_balance: 500.00 }
    ...
    await transactional(async (tx) => {
      ...
      await createOrModifyAccount(tx, task.id, account2)
      await createTransaction(tx, task.id, transaction)
      await createAccountTransaction(tx, task.id, project.id, accountTransaction)
    })
