...
import { BaseEntity, BaseInput, BaseRow, mapRowToType, mapRowToTypeOptional } from '@/app/shared/_helper/DB'
...
export async function findProfilesByOwner(client: PoolClient, owner: string): Promise<Profile[]> {
  const result = await client.query<BaseRow<Profile>>(
...
export async function createOrModifyProfile(client: PoolClient, owner: string, input: ProfileInput): Promise<Profile> {
  ...
  const result = await client.query<BaseRow<Profile>>(query, [input.id, input.text, input.language, owner])
