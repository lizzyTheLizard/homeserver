...
function color(red: number, green: number, blue: number, alpha: number): CSSResult {
  return unsafeCSS(`rgba(${red.toString()}, ${green.toString()}, ${blue.toString()}, ${alpha.toString()})`)
}
...
export function border(color: CSSResult): CSSResult {
  return unsafeCSS(`1px solid ${color.toString()}`)
}
